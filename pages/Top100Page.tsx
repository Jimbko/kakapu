...
const TopAnimeCard: React.FC<{ anime: ShikimoriAnime; rank: number }> = ({ anime, rank }) => {
    return (
        <Link to={`/anime/${anime.id}`} className="flex items-start space-x-4 bg-zinc-800/50 p-4 rounded-lg hover:bg-zinc-700/60 transition-colors duration-200">
            <span className="text-3xl font-black text-zinc-500 w-12 text-center">{rank}</span>
            <img src={anime.image?.preview} alt={anime.russian} className="w-16 h-24 object-cover rounded-md flex-shrink-0" />
            <div className="flex-grow">
                <h3 className="font-bold text-white hover:text-purple-400 transition-colors">{anime.russian || anime.name}</h3>
